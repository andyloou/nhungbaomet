...
//------------------------Parameter----------------------
`define TV_IN_k "../tv/cdatafile/c.Radix2wECC.autotvin_k.dat"
`define TV_IN_x_o "../tv/cdatafile/c.Radix2wECC.autotvin_x_o.dat"
`define TV_IN_y_o "../tv/cdatafile/c.Radix2wECC.autotvin_y_o.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter k_DEPTH = 1;
reg [31 : 0] k_OPERATE_DEPTH = 0;
parameter k_c_bitwidth = 64;
parameter x_o_DEPTH = 1;
reg [31 : 0] x_o_OPERATE_DEPTH = 0;
parameter x_o_c_bitwidth = 64;
parameter y_o_DEPTH = 1;
reg [31 : 0] y_o_OPERATE_DEPTH = 0;
parameter y_o_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter Radix2wECC_continue_addr = 0;
parameter Radix2wECC_auto_start_addr = 0;
parameter k_data_in_addr = 16;
parameter x_o_data_in_addr = 28;
parameter y_o_data_in_addr = 40;
...
reg [x_o_c_bitwidth - 1 : 0] mem_x_o [x_o_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_x_o [ (x_o_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * x_o_DEPTH -1 : 0] = '{default : 'hz};
reg x_o_write_data_finish;
reg [y_o_c_bitwidth - 1 : 0] mem_y_o [y_o_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_y_o [ (y_o_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * y_o_DEPTH -1 : 0] = '{default : 'hz};
reg y_o_write_data_finish;
...
//write x_o reg
reg [31 : 0] write_x_o_count = 0;
reg write_x_o_run_flag = 0;
reg write_one_x_o_data_done = 0;
//write y_o reg
reg [31 : 0] write_y_o_count = 0;
reg write_y_o_run_flag = 0;
reg write_one_y_o_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & k_write_data_finish & x_o_write_data_finish & y_o_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        x_o_write_data_finish <= 0;
        write_x_o_run_flag <= 0; 
        write_x_o_count = 0;
        count_operate_depth_by_bitwidth_and_depth (x_o_c_bitwidth, x_o_DEPTH, x_o_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            x_o_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_x_o_run_flag <= 1; 
            write_x_o_count = 0;
        end
        if (write_one_x_o_data_done === 1) begin
            write_x_o_count = write_x_o_count + 1;
            if (write_x_o_count == x_o_OPERATE_DEPTH) begin
                write_x_o_run_flag <= 0; 
                x_o_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_x_o
    integer write_x_o_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] x_o_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = x_o_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_x_o_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write x_o data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (x_o_c_bitwidth < 32) begin
                        x_o_data_tmp_reg = mem_x_o[write_x_o_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < x_o_c_bitwidth) begin
                                x_o_data_tmp_reg[j] = mem_x_o[write_x_o_count][i*32 + j];
                            end
                            else begin
                                x_o_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_x_o[write_x_o_count * four_byte_num  + i]!==x_o_data_tmp_reg) begin
                    write (x_o_data_in_addr + write_x_o_count * four_byte_num * 4 + i * 4, x_o_data_tmp_reg, write_x_o_resp);
                    image_mem_x_o[write_x_o_count * four_byte_num + i]=x_o_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_x_o_data_done <= 1;
                @(posedge clk);
                write_one_x_o_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        y_o_write_data_finish <= 0;
        write_y_o_run_flag <= 0; 
        write_y_o_count = 0;
        count_operate_depth_by_bitwidth_and_depth (y_o_c_bitwidth, y_o_DEPTH, y_o_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            y_o_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_y_o_run_flag <= 1; 
            write_y_o_count = 0;
        end
        if (write_one_y_o_data_done === 1) begin
            write_y_o_count = write_y_o_count + 1;
            if (write_y_o_count == y_o_OPERATE_DEPTH) begin
                write_y_o_run_flag <= 0; 
                y_o_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_y_o
    integer write_y_o_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] y_o_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = y_o_c_bitwidth;
    process_num = 3;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_3_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_y_o_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write y_o data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (y_o_c_bitwidth < 32) begin
                        y_o_data_tmp_reg = mem_y_o[write_y_o_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < y_o_c_bitwidth) begin
                                y_o_data_tmp_reg[j] = mem_y_o[write_y_o_count][i*32 + j];
                            end
                            else begin
                                y_o_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_y_o[write_y_o_count * four_byte_num  + i]!==y_o_data_tmp_reg) begin
                    write (y_o_data_in_addr + write_y_o_count * four_byte_num * 4 + i * 4, y_o_data_tmp_reg, write_y_o_resp);
                    image_mem_y_o[write_y_o_count * four_byte_num + i]=y_o_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_y_o_data_done <= 1;
                @(posedge clk);
                write_one_y_o_data_done <= 0;
            end   
            process_3_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_x_o_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [x_o_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (x_o_c_bitwidth , factor);
  fp = $fopen(`TV_IN_x_o ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_x_o); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < x_o_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_x_o [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
// Read data from file 
initial begin : read_y_o_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [y_o_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (y_o_c_bitwidth , factor);
  fp = $fopen(`TV_IN_y_o ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_y_o); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < y_o_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_y_o [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
