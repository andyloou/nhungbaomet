...
//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.Radix2wECC.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.Radix2wECC.autotvout_gmem.dat"
...
parameter gmem_mem_depth = 32'd 18;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("C:/HardwareAcceleratedECC-PointMultiplication/ScalarMultiplication/RTL-src/HardwareAcceleratedECC-PointMultiplication/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("C:/HardwareAcceleratedECC-PointMultiplication/ScalarMultiplication/RTL-src/HardwareAcceleratedECC-PointMultiplication/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
  ...
  while(1) begin 
      ...
      for(i = 0; i < (gmem_mem_depth - gmem_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  ...
                  3 : tmp_data = gmem_mem_3[i/factor];
      ...
      if (factor == 4) begin
          case(mem_page)
              ...
              3 : tmp_data = gmem_mem_3[gmem_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((gmem_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  ...
                  3 : tmp_data = gmem_mem_3[gmem_mem_depth/factor][15:0];
                  default: $display("The page_num of write file is not valid!");
              endcase
              bin_data = tmp_data;
              write_binary(fp,bin_data,16);
